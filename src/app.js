...
function render(dt){const moved=updateCamera(dt);if(!state.paused){state.time+=dt;state.touch[2]*=Math.exp(-dt*1.8);state.touch[3]*=Math.exp(-dt*1.8);}if(!ready)return;world.update(state.paused?0:dt,state);renderer.world=world;audio.fuel=state.fuel+world.boost;renderer.render(state,currentCamera(),state.paused?0:dt);updateFuelHUD();dirty=false;return moved;}
...
function setMode(mode){
 ...
 $('gesture-sub').textContent=mode==='feed'?'DRAG & RELEASE':mode==='wind'?'TOUCH THE AIR':small()?'PINCH TO MOVE CLOSER':'SCROLL TO MOVE CLOSER';
...
function pointerMove(e){$('wind-cursor').style.left=`${e.clientX}px`;$('wind-cursor').style.top=`${e.clientY}px`;$('wind-cursor').classList.add('visible');if(!pointers.has(e.pointerId))return;const prev=pointers.get(e.pointerId),dx=e.clientX-prev.x,dy=e.clientY-prev.y;pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});totalDrag+=Math.hypot(dx,dy);if(pointers.size===2){cancelFuelGesture();const p=[...pointers.values()],d=Math.hypot(p[0].x-p[1].x,p[0].y-p[1].y);if(lastPinch>0)cameraGoal.distance=M.clamp(cameraGoal.distance*lastPinch/d,small()?4.8:2.9,8.7);lastPinch=d;dirty=true;return;}lastPinch=0;if(state.mode==='feed'&&!e.altKey){updateFuelGesture(e);return;}if(state.mode==='wind'&&!e.altKey){touchWind(e,dx,dy);return;}cameraGoal.yaw-=dx*.0045;cameraGoal.pitch=M.clamp(cameraGoal.pitch+dy*.0035,.035,.72);dirty=true;}
function pointerDown(e){if(e.button!==0&&e.button!==2)return;try{$('scene').setPointerCapture(e.pointerId);}catch{}pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});totalDrag=0;pointerStart={x:e.clientX,y:e.clientY};if(pointers.size===2){cancelFuelGesture();const p=[...pointers.values()];lastPinch=Math.hypot(p[0].x-p[1].x,p[0].y-p[1].y);}else{lastPinch=0;if(state.mode==='feed'&&!e.altKey)startFuelGesture(e);}}
function pointerUp(e){finishFuelGesture(e,e.type==='pointercancel');pointers.delete(e.pointerId);lastPinch=0;if(!pointers.size&&state.mode==='wind'&&totalDrag<8&&pointerStart)touchWind(e,20,0);pointerStart=null;}
function setupUI(){
 ...
 document.addEventListener('visibilitychange',()=>{cancelFuelGesture();last=0;audio.setPaused(state.paused||document.hidden);dirty=true;});
...
async function start(){try{setupUI();setupFuelUI();await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));renderer=new I.Renderer($('scene'));renderer.world=world;updateCamera(1);
 ...
 ready=true;render(1/60);$('loading').classList.add('done');setTimeout(()=>$('loading').hidden=true,1050);$('gesture-sub').textContent='DRAG & RELEASE';
...
function handleFuelEvent(event){
 ...
 audio.materialEvent?.(event);
 if(event.type==='impact'&&Math.hypot(b.position[0],b.position[2])<1.1){state.touch[0]=b.position[0];state.touch[1]=b.position[1]+.2;state.touch[2]+=event.strength*.18;}
}
...
function setupFuelUI(){
 for(const el of document.querySelectorAll('.material-card')){
  ...
  el.addEventListener('pointermove',updateFuelGesture);
 ...
 addEventListener('blur',cancelFuelGesture);
