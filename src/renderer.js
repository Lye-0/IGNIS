...
class Renderer{
 ...
 updateParticles(dt,state){const r=this.random;this.spawn+=dt*(22+(state.fuel+(this.world?.boost||0))*20+Math.abs(state.touch[2])*12);while(this.spawn>=1&&this.particles.length<200){this.spawn--;const x=(r()-.5)*1.0,z=(r()-.5)*.8;this.particles.push({x,y:.40+r()*.3,z,vx:(r()-.5)*.32,vz:(r()-.5)*.32,vy:.45+r()*.85,age:0,life:.65+r()*2.2,size:.012+r()*.02,seed:r()});}this.spawn=Math.min(this.spawn,1);for(const p of this.particles){p.age+=dt;p.vx+=(state.wind*.45+state.touch[2]*.32-p.vx*.2)*dt;p.vz+=(state.touch[3]*.32-p.vz*.16)*dt;p.x+=(p.vx+Math.sin(p.y*5.+state.time*3.+p.seed*31.)*.16)*dt;p.z+=(p.vz+Math.sin(p.y*4.+state.time*2.+p.seed*12.)*.14)*dt;p.y+=p.vy*dt;p.vy*=Math.exp(-dt*.22);}this.particles=this.particles.filter(p=>p.age<p.life&&p.y<4.4);}
 ...
 burst(event){
  ...
  const count=event.type==='impact'?Math.ceil(b.material.sparks*event.strength):event.type==='ignite'?6:event.count||1;
  for(let i=0;i<count&&this.particles.length<380;i++){
   const ash=(b.kind==='paper'||b.kind==='cardboard')&&event.type==='shed'&&r()>.38;
   this.particles.push({x:b.position[0]+(r()-.5)*.24,y:b.position[1]+.07+r()*.10,z:b.position[2]+(r()-.5)*.23,vx:(r()-.5)*(event.type==='impact'?1.3:.38),vz:(r()-.5)*.8,vy:.35+r()*(event.type==='impact'?1.6:.9),age:0,life:ash?2.2+r()*2.4:.6+r()*1.8,size:ash?.024+r()*.03:.011+r()*.021,seed:r(),kind:ash?1:0});
  }
 }
