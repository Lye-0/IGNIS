...
class Target{
 constructor(gl,width,height,depth=false){this.gl=gl;this.width=width;this.height=height;this.texture=gl.createTexture();gl.bindTexture(gl.TEXTURE_2D,this.texture);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA16F,width,height,0,gl.RGBA,gl.HALF_FLOAT,null);this.framebuffer=gl.createFramebuffer();gl.bindFramebuffer(gl.FRAMEBUFFER,this.framebuffer);gl.framebufferTexture2D(gl.FRAMEBUFFER,gl.COLOR_ATTACHMENT0,gl.TEXTURE_2D,this.texture,0);if(depth){this.depth=gl.createRenderbuffer();gl.bindRenderbuffer(gl.RENDERBUFFER,this.depth);gl.renderbufferStorage(gl.RENDERBUFFER,gl.DEPTH_COMPONENT24,width,height);gl.framebufferRenderbuffer(gl.FRAMEBUFFER,gl.DEPTH_ATTACHMENT,gl.RENDERBUFFER,this.depth);}if(gl.checkFramebufferStatus(gl.FRAMEBUFFER)!==gl.FRAMEBUFFER_COMPLETE)throw new Error('浮動小数点の描画領域を作成できません。');gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT);}
 bind(){const gl=this.gl;gl.bindFramebuffer(gl.FRAMEBUFFER,this.framebuffer);gl.viewport(0,0,this.width,this.height);}
 destroy(){this.gl.deleteTexture(this.texture);this.gl.deleteFramebuffer(this.framebuffer);if(this.depth)this.gl.deleteRenderbuffer(this.depth);}
}
class Renderer{
 constructor(canvas,{quality='medium'}={}){
  ...
  this.fuelRenderer=I.FuelRenderer?new I.FuelRenderer(gl):null;this.world=null;
 ...
 resize(force=false){const w=this.canvas.clientWidth||innerWidth,h=this.canvas.clientHeight||innerHeight;let scale=this.config.scale*Math.min(devicePixelRatio||1,1.65);scale=Math.min(scale,Math.sqrt(this.config.maxPixels/(w*h)));const width=Math.max(2,Math.round(w*scale)),height=Math.max(2,Math.round(h*scale));if(!force&&width===this.width&&height===this.height)return false;this.width=width;this.height=height;this.canvas.width=width;this.canvas.height=height;if(this.targets)for(const t of Object.values(this.targets))t.destroy();const gl=this.gl;this.targets={staticScene:new Target(gl,width,height),scene:new Target(gl,width,height,true),volume:new Target(gl,width,height),bloomA:new Target(gl,Math.max(2,width>>2),Math.max(2,height>>2)),bloomB:new Target(gl,Math.max(2,width>>2),Math.max(2,height>>2)),wideA:new Target(gl,Math.max(2,width>>4),Math.max(2,height>>4)),wideB:new Target(gl,Math.max(2,width>>4),Math.max(2,height>>4))};return true;}
 common(state,camera){return {uResolution:[this.width,this.height],uTime:state.time,uFuel:state.fuel+(this.world?.boost||0),uWind:state.wind,uExposure:state.exposure,uFlare:this.world?.flare||0,uSurgeCount:this.world?.surgeCount||0,uSurgePos:this.world?.surgePositions||new Float32Array(48),uSurgeInfo:this.world?.surgeInfo||new Float32Array(48),uCamera:camera.origin,uForward:camera.forward,uRight:camera.right,uUp:camera.up,uShift:state.shift,uFocal:state.focal,uTouch:state.touch,uGrid:this.grid,uAtlas:this.atlas,uFeedCount:this.world?.sourceCount||0,uFeedPos:this.world?.sources||new Float32Array(48),uFeedInfo:this.world?.sourceInfo||new Float32Array(48),uObjectCount:this.world?.shadowCount||0,uObjectPos:this.world?.shadowPositions||new Float32Array(52)};}
 ...
 updateParticles(dt,state){const r=this.random;this.spawn+=dt*(22+(state.fuel+(this.world?.boost||0))*20+Math.abs(state.touch[2])*12);while(this.spawn>=1&&this.particles.length<200){this.spawn--;const x=(r()-.5)*1.0,z=(r()-.5)*.8;this.particles.push({x,y:.40+r()*.3,z,vx:(r()-.5)*.32,vz:(r()-.5)*.32,vy:.45+r()*.85,age:0,life:.65+r()*2.2,size:.012+r()*.02,seed:r()});}this.spawn=Math.min(this.spawn,1);for(const p of this.particles){p.age+=dt;
  if(dt>0&&this.world){for(let i=0;i<this.world.surgeCount;i++){const k=i*4,a=this.world.surgePositions,b=this.world.surgeInfo,dx=p.x-a[k],dz=p.z-a[k+2],dy=p.y-a[k+1];if(dy>-.1&&dy<2.7){const radius=a[k+3]*(1.3+dy*.4),lift=Math.exp(-(dx*dx+dz*dz)/(radius*radius)-Math.max(0,dy)*.6)*b[k]*b[k+1];p.vy+=lift*dt*.9;}}}
 p.vx+=(state.wind*.45+state.touch[2]*.32-p.vx*.2)*dt;p.vz+=(state.touch[3]*.32-p.vz*.16)*dt;p.x+=(p.vx+Math.sin(p.y*5.+state.time*3.+p.seed*31.)*.16)*dt;p.z+=(p.vz+Math.sin(p.y*4.+state.time*2.+p.seed*12.)*.14)*dt;p.y+=p.vy*dt;p.vy*=Math.exp(-dt*.22);}this.particles=this.particles.filter(p=>p.age<p.life&&p.y<4.4);}
 drawParticles(uniforms){if(!this.particles.length)return;const gl=this.gl;let n=0;for(const p of this.particles){const life=clamp(1-p.age/p.life,0,1)*Math.min(1,p.age*20);for(let j=0;j<3;j++){const off=n++*8;this.sparkData.set([p.x-p.vx*j*.012,p.y-p.vy*j*.014,p.z-p.vz*j*.012,p.size*(1-j*.15),life*(1-j*.24),p.seed,p.kind||0,0],off);}}gl.bindVertexArray(this.sparkVao);gl.bindBuffer(gl.ARRAY_BUFFER,this.sparkBuffer);gl.bufferSubData(gl.ARRAY_BUFFER,0,this.sparkData.subarray(0,n*8));this.programs.sparks.use().values(uniforms).texture('uScene',this.targets.scene.texture,0);gl.enable(gl.BLEND);gl.blendFunc(gl.ONE,gl.ONE_MINUS_SRC_ALPHA);gl.colorMask(true,true,true,false);gl.drawArrays(gl.POINTS,0,n);gl.colorMask(true,true,true,true);gl.disable(gl.BLEND);gl.bindVertexArray(this.vao);}
 ...
 render(state,camera,dt){if(this.destroyed)return;const gl=this.gl;this.resize();const u=this.common(state,camera);gl.disable(gl.DEPTH_TEST);gl.disable(gl.BLEND);this.simulate(dt,u);
  ...
  T.staticScene.bind();this.programs.scene.use().values(u).texture('uField',field,1).texture('uNoise',this.noise,2,gl.TEXTURE_3D);this.quad();
  gl.bindFramebuffer(gl.READ_FRAMEBUFFER,T.staticScene.framebuffer);gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER,T.scene.framebuffer);gl.blitFramebuffer(0,0,this.width,this.height,0,0,this.width,this.height,gl.COLOR_BUFFER_BIT,gl.NEAREST);
  T.scene.bind();gl.clearDepth(1);gl.clear(gl.DEPTH_BUFFER_BIT);
  if(this.world&&this.fuelRenderer)this.fuelRenderer.draw(this.world,u,T.staticScene.texture,field,this.noise);
 ...
 burst(event){
  const b=event.body,r=this.random;
  const profile=b.material.response;
  if(event.type==='impact'&&(event.exposure??1)<=.02)return;
  const count=event.type==='impact'?Math.ceil(b.material.sparks*event.strength):event.type==='ignite'?Math.ceil(profile.sparks*Math.min(1.15,(event.strength||profile.strength)/profile.strength)):event.count||1;
  for(let i=0;i<count&&this.particles.length<380;i++){
   const ash=(b.kind==='paper'||b.kind==='cardboard')&&event.type==='shed'&&r()>.38;
   this.particles.push({x:b.position[0]+(r()-.5)*.24,y:b.position[1]+.07+r()*.10,z:b.position[2]+(r()-.5)*.23,vx:(r()-.5)*(event.type==='impact'?1.3:.38),vz:(r()-.5)*.8,vy:.35+r()*(event.type==='impact'?1.6:event.type==='ignite'?1.6: .9),age:0,life:ash?2.2+r()*2.4:.6+r()*1.8,size:ash?.024+r()*.03:.011+r()*.021,seed:r(),kind:ash?1:0});
  }
 }
 reset(){this.initialize=true;this.particles=[];this.spawn=0;}
 info(){return {quality:this.quality,resolution:[this.width,this.height],grid:this.grid,volumeSteps:this.config.steps,frame:this.frame,particles:this.particles.length,fuel:this.world?.info()||null};}
 destroy(){if(this.destroyed)return;this.destroyed=true;this.fuelRenderer?.destroy();for(const p of Object.values(this.programs))p.destroy();for(const t of Object.values(this.targets))t.destroy();for(const f of this.fields)f.destroy();this.advected.destroy();const gl=this.gl;gl.deleteTexture(this.noise);gl.deleteBuffer(this.sparkBuffer);gl.deleteVertexArray(this.vao);gl.deleteVertexArray(this.sparkVao);}
}
I.GLProgram=Program;I.Renderer=Renderer;I.QUALITY=QUALITY;
